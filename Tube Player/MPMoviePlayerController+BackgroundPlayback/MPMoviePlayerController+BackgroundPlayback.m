...
+ (void) backgroundPlayback_applicationWillResignActive:(NSNotification *)notification
{
	...
	if (currentMoviePlayerController.isBackgroundPlaybackEnabled)
	{
        ...
        NSTimeInterval finish = CACurrentMediaTime();
        NSTimeInterval start = [[[NSUserDefaults standardUserDefaults] objectForKey:@"lastPause"] doubleValue];
        double difference = finish - start;
        if (difference <= 0.1)
        {
            [currentMoviePlayerController play];
        }
    }
}
